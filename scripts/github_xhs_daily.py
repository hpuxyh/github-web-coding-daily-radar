...
PUBLIC_DAILY_DIR = PROJECT_ROOT / "public" / "daily"
...
def daily_archive_entry(payload: dict[str, Any]) -> dict[str, Any]:
    date_text = str(payload.get("date") or "").strip()
    return {
        "date": date_text,
        "path": f"{date_text}.json",
        "repo_count": len(payload.get("all_repos") or []),
        "hot_count": len(payload.get("hot") or []),
        "used_count": len(payload.get("used") or []),
        "starred_count": len(payload.get("starred") or []),
        "discussion_count": len(payload.get("discussion") or []),
        "draft_count": len(payload.get("xhs_repos") or []),
    }


def load_daily_archive_index(path: Path) -> dict[str, Any]:
    if not path.exists():
    ...
    if not isinstance(loaded, dict):
        return {"updated_at": "", "dates": []}
    dates = [item for item in loaded.get("dates") or [] if isinstance(item, dict) and item.get("date")]
    return {"updated_at": str(loaded.get("updated_at") or ""), "dates": dates}


def write_public_daily_archive(payload: dict[str, Any], archive_dir: Path = PUBLIC_DAILY_DIR) -> tuple[Path, Path]:
    date_text = str(payload.get("date") or "").strip()
    if not date_text:
        raise ValueError("daily archive payload is missing date")
    dt.date.fromisoformat(date_text)

    archive_dir.mkdir(parents=True, exist_ok=True)
    day_path = archive_dir / f"{date_text}.json"
    ...
    entry = daily_archive_entry(payload)
    by_date = {str(item.get("date")): item for item in index.get("dates") or [] if item.get("date")}
    by_date[date_text] = entry
    dates = sorted(by_date.values(), key=lambda item: str(item.get("date") or ""), reverse=True)
    index_payload = {
        "updated_at": dt.datetime.utcnow().replace(microsecond=0).isoformat() + "Z",
        "latest_date": dates[0]["date"] if dates else date_text,
        "dates": dates,
    }
...
def write_outputs(
    ...
    archive_path, archive_index_path = write_public_daily_archive(payload)
    print(f"Daily archive: {archive_path}")
    print(f"Daily archive index: {archive_index_path}")
...
def embed_latest_json(args: argparse.Namespace) -> int:
    ...
    if payload.get("date"):
        archive_path, archive_index_path = write_public_daily_archive(payload)
        print(f"Updated daily archive: {archive_path}")
        print(f"Updated daily archive index: {archive_index_path}")
